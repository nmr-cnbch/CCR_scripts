...
def read_ucsf_info(filename):
    ...
    with open(filename, "rb") as ucsf_file:
        ...
        print ("Spectra dimensiolity:", spectra_dim)
        ...
        print ("Number of points in axis:",*points_num)
        ...
        print ("Spectrometer frequency (MHz): ",' '.join("{:.2f}".format(x) for x in spectr_fq))
        ...
        sw.insert(0,sw.pop())
        print ("Spectral width (Hz):", ' '.join("{:.0f}".format(x) for x in sw))
    ...
    print ("Spectral width (ppm):", ' '.join("{:.1f}".format(x) for x in sw_ppm))
...
def ppm2points(ppm, points_num, center_data, sw_ppm, output_format): 
    ...
    if point_value > points_num:
        point_value = points_num - point_value
    if point_value < 0:
        point_value = points_num + point_value
...
def calc_peak_points(points_num, peaks, sw_ppm, center_data,s_dim,num_format="Integer"):
    for peak_num in range(len(peaks)):
        for p in range(len(peaks[peak_num].peak_ppm_pos)):
            one_dim = ppm2points(peaks[peak_num].peak_ppm_pos[p],points_num[p],center_data[p],sw_ppm[p],num_format)
...
if __name__ == "__main__":
    ...
    Spectra_dim, Nucl_name, Points_number, Tile_size, Spectrometer_fq, SW_size, SW_size_ppm, Centr_data, N_Tiles = read_ucsf_info (filename)
